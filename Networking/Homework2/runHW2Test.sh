...
echo "*** Test 1: Simple DNS query with single answer ***"
CSIP=`./dnsq @8.8.8.8 www.cs.clemson.edu | sed '/^$/d' | tail -n 1 | awk '{print $2}' & sleep 2; killall -q -9 dnsq &> /dev/null` 
GRADIP=`./dnsq @8.8.8.8 www.grad.clemson.edu | sed '/^$/d' | tail -n 1 | awk '{print $2}' & sleep 2; killall -q -9 dnsq &> /dev/null` 
...
CSTTL=`./dnsq @8.8.8.8 www.cs.clemson.edu 2>&1 | grep 3600 | wc -l & sleep 2; killall -q -9 dnsq &> /dev/null` 
...
EBAYIPS=`./dnsq @8.8.8.8 www.ebay.com | grep IP -c & sleep 2; killall -q -9 dnsq &> /dev/null`
